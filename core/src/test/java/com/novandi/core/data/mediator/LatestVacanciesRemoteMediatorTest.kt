...
class LatestVacanciesRemoteMediatorTest {
    ...
    private lateinit var mediator: LatestVacanciesRemoteMediator

    @Before
    fun setup() {
        MockKAnnotations.init(this)
        coEvery { localDataSource.deleteLatestVacancies() } just Runs
        mediator = LatestVacanciesRemoteMediator(
            remoteDataSource,
            localDataSource,
            testAppExecutors
        )
    ...
    fun `refresh load returns success result when vacancies are fetched successfully`() = runTest {
        ...
        coEvery { remoteDataSource.getLatestVacancies(1, 10) } returns vacancyFeed
        coEvery { localDataSource.insertLatestVacancy(any()) } just Runs

        val pagingState = PagingState<Int, LatestVacancyEntity>(
            listOf(),
            null,
            PagingConfig(10),
            10
    ...
    fun `refresh load returns error result when an exception occurs`() = runTest {
        coEvery { remoteDataSource.getLatestVacancies(any(), any()) } throws IOException()
    ...
    fun `append load returns success result when vacancies are fetched successfully`() = runTest {
        ...
        val lastItem = VacancyMapper.mapResponseToLatestVacancyEntity(vacancyFeed).last()
