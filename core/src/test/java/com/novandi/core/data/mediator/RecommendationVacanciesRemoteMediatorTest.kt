...
class RecommendationVacanciesRemoteMediatorTest {
    ...
    private lateinit var mediator: RecommendationVacanciesRemoteMediator
    private val recommendationRequest = RecommendationRequest(
    ...
    fun setup() {
        ...
        coEvery { localDataSource.deleteRecommendation() } just Runs
        mediator = RecommendationVacanciesRemoteMediator(
            remoteDataSource,
            localDataSource,
            testAppExecutors,
    ...
    fun `refresh load returns success result when vacancies are fetched successfully`() = runTest {
        ...
        coEvery { remoteDataSource.getRecommendation(recommendationRequest, 1, 10) } returns vacancyFeed
        coEvery { localDataSource.insertRecommendation(any()) } just Runs

        val pagingState = PagingState<Int, RecommendationVacancyEntity>(
            listOf(),
            null,
            PagingConfig(10),
            10
    ...
    fun `refresh load returns error result when an exception occurs`() = runTest {
        coEvery { remoteDataSource.getRecommendation(any(), any(), any()) } throws IOException()
    ...
    fun `append load returns success result when vacancies are fetched successfully`() = runTest {
        ...
        val lastItem = VacancyMapper.mapResponseToRecommendationVacancyEntity(vacancyFeed).last()
