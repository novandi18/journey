...
class LatestVacanciesRemoteMediator(
    private val remoteDataSource: RemoteDataSource,
    private val localDataSource: LocalDataSource,
    private val appExecutors: AppExecutors
): RemoteMediator<Int, LatestVacancyEntity>() {
    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, LatestVacancyEntity>
    ): MediatorResult {
        return try {
            ...
            appExecutors.diskIO().execute {
                if (loadType == LoadType.REFRESH) {
                    localDataSource.deleteLatestVacancies()
                }

                val vacancyEntities = VacancyMapper.mapResponseToLatestVacancyEntity(vacancyFeed)
                localDataSource.insertLatestVacancy(vacancyEntities)
            }
